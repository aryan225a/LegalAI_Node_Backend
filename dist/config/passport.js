...
passport.use('jwt-citizen', new JwtStrategy({
    jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken(),
    secretOrKey: JWT_SECRET,
}, async (payload, done) => {
    try {
        if (payload.userType !== 'CITIZEN')
...
passport.use('jwt-lawyer', new JwtStrategy({
    jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken(),
    secretOrKey: JWT_SECRET,
}, async (payload, done) => {
    try {
...
passport.use('jwt-firm', new JwtStrategy({
    jwtFromRequest: ExtractJwt.fromAuthHeaderAsBearerToken(),
    secretOrKey: JWT_SECRET,
}, async (payload, done) => {
    try {
        if (payload.userType !== 'FIRM_ADMIN')
            return done(null, false);
        if (!payload.twoFactorVerified)
            return done(null, false);
