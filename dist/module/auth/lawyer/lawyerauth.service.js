...
import { validateLawyerRegistration, validatePasswordStrength } from '../../../utils/validator.js';
import { generateLawyerTokens, generateTwoFaTempToken, verifyTwoFaTempToken, verifyRefreshToken, } from '../../../utils/jwt.utils.js';
class LawyerAuthService {
    async register(input) {
        const { email, password, name, phone, barNumber, barCouncilState, practiceAreas = [], yearsOfExperience, } = input;
        ...
        try {
            const otp = await otpService.generate(lawyer.id, 'lawyer', 'EMAIL_VERIFICATION');
    ...
    async loginStep1(email, password) {
        ...
        if (lawyer.verificationStatus === 'FORMAT_FAILED') {
            throw new AppError('Account registration has validation errors. Please contact support.', 403, 'ACCOUNT_NOT_VERIFIED');
        }
    ...
    async loginStep2(twoFactorToken, otp, ipAddress) {
        ...
        const { accessToken, refreshToken } = generateLawyerTokens(lawyerId, lawyer.verificationStatus);
    ...
    async refreshTokens(refreshToken) {
        ...
        const { accessToken, refreshToken: newRefreshToken } = generateLawyerTokens(lawyer.id, lawyer.verificationStatus);
        await this.storeRefreshToken(lawyer.id, newRefreshToken);
        return { accessToken, refreshToken: newRefreshToken };
    }
    async logout(lawyerId, refreshToken) {
    ...
    async handleFailedLogin(lawyerId, currentAttempts) {
    ...
    async storeRefreshToken(lawyerId, token, ipAddress) {
...
//# sourceMappingURL=lawyerauth.service.js.map