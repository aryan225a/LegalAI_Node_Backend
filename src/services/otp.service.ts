...
class OtpService {
  ...
  private generateRawOtp(): string {
    const otp = randomInt(0, 999999);
    return otp.toString().padStart(OTP_DIGITS, '0');
  }

  private async enforceHourlyLimit(userId: string, userType: OtpUserType): Promise<void> {
  ...
  private async incrementHourlyCount(userId: string, userType: OtpUserType): Promise<void> {
    const key = `otp:hourly:${userType}:${userId}`;
  ...
  private async incrementAttempts(recordId: string, userType: OtpUserType): Promise<void> {
    if (userType === 'lawyer') {
  ...
  private async markUsed(recordId: string, userType: OtpUserType): Promise<void> {
    if (userType === 'lawyer') {
      await prisma.otpRecord.update({
        where: { id: recordId },
  ...
  private async killRecord(recordId: string, userType: OtpUserType): Promise<void> {
    if (userType === 'lawyer') {
      await prisma.otpRecord.update({
        where: { id: recordId },
