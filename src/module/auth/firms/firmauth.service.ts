...
class FirmAuthService {
  ...
  async register(input: {
    ...
    const {
      email, password, name, firmName,
      phone, registrationNumber, gstNumber,
      city, state, address,
    } = input;
    ...
    const encryptedRegistrationNumber = encryptionService.encrypt(
  ...
  async verifyEmail(firmId: string, otp: string) {
  ...
  async loginStep1(
    ...
    if (firm.isLocked) {
      throw new AppError(
        `Account is locked. Reason: ${firm.lockReason || 'Security hold'}`,
        403,
        'ACCOUNT_LOCKED'
      );
    }
    ...
    if (firm.verificationStatus === 'FORMAT_FAILED') {
      throw new AppError(
        'Account has registration validation errors. Please contact support.',
        403,
  ...
  async loginStep2(twoFactorToken: string, otp: string, ipAddress?: string) {
    ...
    const { accessToken, refreshToken } = generateFirmTokens(
      firmId,
  ...
  async refreshTokens(refreshToken: string) {
    ...
    const { accessToken, refreshToken: newRefreshToken } = generateFirmTokens(
      firm.id,
      firm.verificationStatus
    );
  ...
  async logout(firmId: string, refreshToken: string) {
  ...
  private async handleFailedLogin(firmId: string, currentAttempts: number) {
